# GUI FILE
from ComSysTrainerUI import Ui_ComSys
...
class MainWindow(QMainWindow):
    ...
    def modulateAM(self):
        fc = float(self.ui.carrierFreqInput.text())
        fm = float(self.ui.msgFreqInput.text())
        t  =  np .arange(44100.0)/44100.0

        ac = float(self.ui.carrierAmpInput.text())
        am = float(self.ui.msgAmpInput.text())
        ...
        self.ui.MplWidgetAM.canvas.ax1.clear()
        self.ui.MplWidgetAM.canvas.ax1.plot(t,message_signal,color='cyan',label=f'Message\n Amp: {am} Freq: {fm} Hz')
        self.ui.MplWidgetAM.canvas.ax1.legend(loc = 'lower right')
        
        # Plotting carrier signal
        self.ui.MplWidgetAM.canvas.ax2.clear()
        self.ui.MplWidgetAM.canvas.ax2.plot(t,carrier_signal,color='lime',label=f'Carrier\n Amp: {ac} Freq: {fc} Hz')
        self.ui.MplWidgetAM.canvas.ax2.legend(loc = 'lower right')
        
        # Plotting AM modulated signal
        self.ui.MplWidgetAM.canvas.ax3.clear()
        self.ui.MplWidgetAM.canvas.ax3.plot(t,am_product,color='orange',label=f'AM Signal Vmax/Vmin = {ac+am}/{ac-am} \n USBF: {fc+fm} Hz LSBF: {fc-fm} Hz')
        ...
        self.ui.MplWidgetAM.canvas.ax1.set_title(f'AM signal plot : Mod Index = {mod_index}')
        self.ui.MplWidgetAM.canvas.draw()
    ...
    def modulateASK(self):
        ...
        if(self.checkMessage(message,self.ui.messageInputASK)):
            ...
            self.ui.MplWidgetASK.canvas.ax1.clear()
            self.ui.MplWidgetASK.canvas.ax1.plot(t,data_signal,color='cyan',label=f'Sampling Freq: {Fs} Hz\n Message: {message}')
            self.ui.MplWidgetASK.canvas.ax1.legend(loc = 'lower right')

            # Plotting carrier signal
            self.ui.MplWidgetASK.canvas.ax2.clear()
            self.ui.MplWidgetASK.canvas.ax2.plot(t,carrier_signal,color='lime',label=f'Carrier Freq: {Fc} Hz')
            self.ui.MplWidgetASK.canvas.ax2.legend(loc = 'lower right')
            
            # Plotting ASK modulated signal
            self.ui.MplWidgetASK.canvas.ax3.clear()
            self.ui.MplWidgetASK.canvas.ax3.plot(t,ask_signal,color='orange',label=f'ASK Signal \nFreq = 0 Hz: Data(0)\n Freq = {Fc} Hz: Data(1)')
            ...
            # Set Title and Show Plot
            self.ui.MplWidgetASK.canvas.ax1.set_title(f'ASK signal plot')
            self.ui.MplWidgetASK.canvas.draw()
